      ...
      <Table className="w-full bg-white">
        <TableHeader>
          <TableRow className="bg-gray-50/50 backdrop-blur-sm">
            <TableHead className="w-[200px] px-3 font-semibold sticky left-0 z-20 bg-gray-50/50">特征</TableHead>
            {rooms.map((room) => (
              <TableHead 
                key={room.id}
          ...
          {featureGroups.map((group) => (
            <React.Fragment key={group.title}>
              ...
              {group.features.map((feature) => (
                ...
                >
                  <TableCell className="w-[200px] px-3 font-medium text-gray-700 bg-gray-50/30 whitespace-nowrap sticky left-0 z-20">
                    {feature.label}
                  </TableCell>
                  {rooms.map((room) => (
                    <TableCell 
